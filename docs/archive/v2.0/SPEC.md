> **Archived:** superseded by [`../../SPEC.md`](../../SPEC.md). Do not implement from this v2.0 document.
...
| Languages | **TypeScript + Python, exclusively** | TS for all user-facing code (PWA, map site, edge functions) — one language, largest OSS contributor pool. Python for pipeline + ML — the geospatial/ML ecosystem lives there. Flutter/Dart from the v1 spec is removed entirely. |
...
- GitHub org (proposed: `global-soil-intelligence`), monorepo `gsip`, public roadmap via GitHub Projects, discussions on.
...
- Existing repo `github.com/jhumoz/GSIP` to be reviewed and either archived or migrated into the org.

...
6. **No paid/closed services** in the default deployment path. Supabase free tier, GitHub Actions, GitHub Pages, Hugging Face free tier, Open-Meteo, SoilGrids REST. Anything with a bill or a proprietary SDK needs product-owner sign-off first. *Known upcoming decision:* Supabase free tier (500 MB database / 1 GB storage / 7-day inactivity pause) suffices for Phase 0 development; a Pro-tier upgrade decision is expected before the Phase 1 public launch — product owner signs off.
...
*Builder: read `docs/SPEC.md` in full before WP-0. When in doubt, open an issue — a question costs minutes; an assumption costs a rebuild.*
